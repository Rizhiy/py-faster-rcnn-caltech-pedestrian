...
#VGG16 Name of pre-trained net to be used
#For testing, this is only used for getting the directory name where .pt file is located
NET=$2 
NET_lc=${NET,,}
...
#take absolute path of .caffemodel file as an agrument
NET_FINAL=$4
...
LOG="experiments/logs/faster_rcnn_test_00_${NET}_${EXTRA_ARGS_SLUG}.txt.`date +'%Y-%m-%d_%H-%M-%S'`"
exec &> >(tee -a "$LOG")
